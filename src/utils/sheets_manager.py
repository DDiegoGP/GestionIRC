...
class SheetsManager:
    ...
    def append_rows(self, sheet_name: str, values: List[List[Any]]) -> bool:
        ...
        try:
            ...
            # Limpiar caché para esta hoja
            cache_key = f"{self.spreadsheet_id}_{sheet_name}"
            if cache_key in self.cache:
                del self.cache[cache_key]
    ...
    def append_row(self, sheet_name: str, row: List[Any]) -> bool:
        """
        Añade una sola fila al final de la hoja.
        
        Args:
            sheet_name: Nombre de la hoja
            row: Lista con los valores de la fila
    ...
    def update_row(self, sheet_name: str, row_index: int, row: List[Any]) -> bool:
        """
        Actualiza una fila específica.
        
        Args:
            sheet_name: Nombre de la hoja
            row_index: Índice de la fila (1-based, incluyendo encabezados)
            row: Lista con los nuevos valores
            
        Returns:
            bool: True si actualización exitosa
        """
        try:
            # Convertir índice a rango
            ...
            logger.info(f"✅ Actualizada fila {row_index} en {sheet_name}")
            return True
            
        except HttpError as e:
            logger.error(f"Error al actualizar fila {row_index} en {sheet_name}: {e}")
            return False
    ...
    def delete_row(self, sheet_name: str, row_index: int) -> bool:
        ...
        try:
            ...
            if sheet_id is None:
                logger.error(f"No se encontró la hoja {sheet_name}")
                return False
            
            # Eliminar fila (índice 0-based para la API de batchUpdate)
            requests = [{
                'deleteDimension': {
                    'range': {
                        'sheetId': sheet_id,
            ...
            logger.info(f"✅ Eliminada fila {row_index} en {sheet_name}")
            return True
            
        except HttpError as e:
            logger.error(f"Error al eliminar fila {row_index} en {sheet_name}: {e}")
            return False
